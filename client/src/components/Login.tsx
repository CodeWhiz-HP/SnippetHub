    ...
    return (
        <div className="flex flex-col gap-2">
            <form onSubmit={handleLogin} className="flex flex-col gap-4 login-form w-[30%] h-[48%] p-4 m-2 mb-6 bg-white rounded-md">
                ...
                <button className="bg-indigo-600 text-white px-2 py-1 rounded hover:bg-indigo-700 hover:cursor-pointer w-fit mt-4">Submit</button>
            </form>
            <form >
                <input className="text-lg p-3 rounded-md h-10 bg-gray200" value={code} onChange={(e) => setCode(e.target.value)} placeholder="6-digit code" />
                <button className="bg-indigo-600 text-white px-2 py-1 rounded hover:bg-indigo-700 hover:cursor-pointer w-fit mt-4">Login</button>
            </form>
        </div>
    );
};